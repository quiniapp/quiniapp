 ...
 */
export async function initializeActiveDaysCache() {
    try {
        ...
        console.log(`[ArchiveHelper] Cache initialized with ${activeDays.length} active days:`, activeDays);
    }
    catch (error) {
 ...
 */
export async function refreshActiveDaysCache() {
    try {
        const activityRepo = new ActivityDaysRepository();
        const daysToKeep = getDaysToKeep();
        const activeDays = await activityRepo.getLastActiveDays(daysToKeep);
        cachedActiveDays = activeDays;
        lastCacheUpdate = new Date();
        console.log(`[ArchiveHelper] Cache refreshed with ${activeDays.length} active days:`, activeDays);
    }
    catch (error) {
 ...
 */
export function isArchiveDate(date) {
 ...
 */
export function getTableName(date, baseTable) {
 ...
 */
export function getRpcName(date, baseRpcName) {
    const isArchived = isArchiveDate(date);
