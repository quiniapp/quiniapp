...
export class ArchiveService {
     ...
     */
    async archiveOldBets(daysToKeep = 2) {
        try {
     ...
     * Uses stored procedure if available, fallback to TypeScript implementation
     */
    async archiveOldTickets(daysToKeep = 2) {
        try {
            // Try using stored procedure first (performance optimization)
     ...
     */
    async archiveOldData(daysToKeep = 2) {
        try {
     ...
     */
    async archiveOldBetsManual(daysToKeep) {
        ...
        if (selectError) {
            const errorMsg = typeof selectError.message === 'string' ? selectError.message : JSON.stringify(selectError);
        ...
        if (insertError) {
            const errorMsg = typeof insertError.message === 'string' ? insertError.message : JSON.stringify(insertError);
        ...
        if (deleteError) {
            const errorMsg = typeof deleteError.message === 'string' ? deleteError.message : JSON.stringify(deleteError);
     ...
     */
    async archiveOldTicketsManual(daysToKeep) {
        const cutoffDate = await this.activityDaysRepo.getCutoffDate(daysToKeep);
        if (!cutoffDate) {
            return {
                success: true,
     ...
     */
    async archiveOldDataManual(daysToKeep) {
