...
const getBaseUserFields = (user, organization_id) => {
    ...
    const shouldNumberBeNull = user.user_type === USER_TYPE.OWNER ||
...
export const buildUserForDB = async (user, organization_id) => {
    const baseUser = getBaseUserFields(user, organization_id);
    // Validar que ADMIN y CASHIER tengan número
    if ((user.user_type === USER_TYPE.ADMIN || user.user_type === USER_TYPE.CASHIER) &&
