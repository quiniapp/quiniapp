...
import { hashPassword, generateRandomPassword, validatePasswordStrength, comparePassword, } from 'api/helper/password';
...
export class UserController {
    constructor() {
         ...
         */
        this.canManageUser = (adminType, targetType) => {
            return USER_HIERARCHY[adminType] < USER_HIERARCHY[targetType];
        };
        this.create = async (newUser, organization_id) => {
         ...
         */
        this.getByIdFromNetwork = async (userId, adminOrgId) => {
        ...
        this.update = async (user_id, props, organization_id) => {
         ...
         */
        this.validateCapitalist = async (username, organization_id, adminUserType) => {
         ...
         */
        this.resetPassword = async (targetUserId, adminUserId, adminUserType, adminOrgId, newPassword) => {
            // Check base permissions - only OWNER, CAPITALIST, SUPERADMIN, ADMIN can reset passwords
            if (![USER_TYPE.OWNER, USER_TYPE.CAPITALIST, USER_TYPE.SUPERADMIN, USER_TYPE.ADMIN].includes(adminUserType)) {
            ...
            if (adminUserType === USER_TYPE.OWNER) {
                ...
                if (!descendantOrgs.includes(targetUser.organization_id)) {
                    throw new ForbiddenError('Solo puedes resetear contraseñas de usuarios de tu organización o sub-organizaciones');
                }
            }
            else {
            ...
            if (!this.canManageUser(adminUserType, targetUser.user_type)) {
                throw new ForbiddenError('No puedes resetear la contraseña de un usuario de igual o mayor jerarquía');
            }
            ...
            switch (adminUserType) {
                case USER_TYPE.OWNER:
                    ...
                    if ([USER_TYPE.SUPERADMIN, USER_TYPE.ADMIN, USER_TYPE.CASHIER].includes(targetUser.user_type)) {
                        // Must be from same organization
                        if (targetUser.organization_id !== adminOrgId) {
                            throw new ForbiddenError('Solo puedes resetear contraseñas de usuarios de tu organización');
                        }
                    }
                    break;
                ...
                case USER_TYPE.ADMIN:
                    ...
                    if (targetUser.organization_id !== adminOrgId) {
                        throw new ForbiddenError('Solo puedes resetear contraseñas de cajeros de tu organización');
                    }
                    break;
                default:
                    throw new ForbiddenError('No tienes permisos para resetear contraseñas');
         ...
         */
        this.changePassword = async (userId, currentPassword, newPassword, organization_id) => {
            // Get user
         ...
         */
        this.unlockAccount = async (targetUserId, adminUserId, adminUserType, adminName, adminOrgId, ipAddress, userAgent) => {
            ...
            if (adminUserType === USER_TYPE.OWNER) {
                ...
                targetUser = await this.repository.getByIdWithoutOrgRestriction(targetUserId);
                if (!descendantOrgs.includes(targetUser.organization_id)) {
                    throw new ForbiddenError('Solo puedes desbloquear cuentas de usuarios de tu organización o sub-organizaciones');
                }
            }
            else {
                targetUser = await this.repository.getById(targetUserId, adminOrgId);
         ...
         */
        this.getNetworkOrgIds = async (organizationId) => {
         ...
         */
        this.assignUserToGroup = async (userId, targetGroupId, adminOrgId, adminUserType) => {
            ...
            const result = await this.repository.assignToGroup(userId, targetUser.organization_id, targetGroupId);
            return parseUser(result);
        };
        /**
         ...
         */
        this.getUsersForGroupAssignment = async (adminOrgId, adminUserType, excludeGroupId) => {
    ...
    async getAll(organization_id, user_type, cashier_number, filter_user_type, include_session) {
        // OWNER can see all users from all organizations (for password reset)
        // Others only see users from their own organization
        const result = await this.repository.getAll(organization_id, user_type, cashier_number, filter_user_type, include_session);
