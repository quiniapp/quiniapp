...
export class UserRepository {
  ...
  `
     ...
     */
    async getOrganizationDescendants(organizationId) {
        const { data, error } = await supabase.rpc('get_organization_descendants', {
            p_org_id: organizationId,
        });
        if (error)
            throw new Error(error.message);
        return data?.map((d) => d.organization_id) || [organizationId];
    }
    /**
     * Check if an organization is a sub-organization (has a parent)
     */
    async isSubOrganization(organizationId) {
     ...
     */
    async getByIdWithoutOrgRestriction(id) {
        const { data, error } = await supabase
            .from('users')
            .select(this.allUserFields)
            .eq('user_id', id)
    ...
    async getByUsernameAndOrganization(username, organization_id) {
        const { data, error } = await supabase
            .from('users')
            .select(this.allUserFields)
     ...
     */
    async assignToGroup(userId, currentOrgId, targetOrgId) {
        const timestamp = dayjs().toISOString();
        const { data, error } = await supabase
            .from('users')
     ...
     */
    async getUsersForGroupAssignment(networkOrgIds, excludeOrgId) {
        let query = supabase
            .from('users')
            .select('*')
