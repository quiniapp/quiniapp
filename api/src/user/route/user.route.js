...
export class UserRouter {
    constructor() {
        this.newUserhandler = asyncHandler(async (req, res) => {
        ...
        this.getUserHandler = asyncHandler(async (req, res) => {
        ...
        this.getAllUserHandler = asyncHandler(async (req, res) => {
            const { user } = req;
            ...
            const users = await this.controller.getAll(targetOrgId, user.user.user_type, parsedCashierNumber, filterUserType, includeSession);
            const response = {
                data: {
                    users,
                },
        ...
        this.updateUserHandler = asyncHandler(async (req, res) => {
            const { id: user_id } = req.params;
        ...
        this.deleteUserHandler = asyncHandler(async (req, res) => {
            const { id: user_id } = req.params;
            const { user } = req;
            if (!user_id) {
                throw new BadRequestError('ID de usuario requerido');
         ...
         */
        this.resetPasswordHandler = asyncHandler(async (req, res) => {
            ...
            if (![USER_TYPE.OWNER, USER_TYPE.CAPITALIST, USER_TYPE.SUPERADMIN, USER_TYPE.ADMIN].includes(user.user.user_type)) {
                throw new ForbiddenError('No tienes permisos para resetear contraseñas');
            }
            const result = await this.controller.resetPassword(targetUserId, user.user.user_id, user.user.user_type, req.organization_id, newPassword);
            const response = {
                data: {
         ...
         */
        this.changePasswordHandler = asyncHandler(async (req, res) => {
            ...
            await this.controller.changePassword(user.user.user_id, currentPassword, newPassword, req.organization_id);
            const response = {
                data: {
                    success: true,
                },
         ...
         */
        this.unlockAccountHandler = asyncHandler(async (req, res) => {
            const { id: targetUserId } = req.params;
            const { user } = req;
            if (!targetUserId) {
                throw new BadRequestError('ID de usuario requerido');
            ...
            await this.controller.unlockAccount(targetUserId, user.user.user_id, user.user.user_type, user.user.name, req.organization_id, req.ip, req.get('user-agent'));
            const response = {
                data: { success: true },
            };
            res.status(200).json(response);
         ...
         */
        this.validateCapitalistHandler = asyncHandler(async (req, res) => {
            ...
            const result = await this.controller.validateCapitalist(username, organization_id, user.user.user_type);
            const response = {
                data: {
         ...
         */
        this.getAssignableUsersHandler = asyncHandler(async (req, res) => {
            const { exclude_group_id } = req.query;
            const { user } = req;
            if (!user) {
                throw new ForbiddenError('No autenticado');
            }
            const users = await this.controller.getUsersForGroupAssignment(req.organization_id, user.user.user_type, exclude_group_id);
            const response = {
                data: {
                    users,
                },
         ...
         */
        this.assignUserToGroupHandler = asyncHandler(async (req, res) => {
            ...
            const result = await this.controller.assignUserToGroup(user_id, group_id, req.organization_id, user.user.user_type);
            const response = {
                data: {
                    user: result,
                },
