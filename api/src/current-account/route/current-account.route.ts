...
import { CurrentAccountController } from '../controller/currentaccount.controller';
import { APIResponse } from '@helper/response/api_response.response';
import { ERROR_MESSAGE, ERROR_TYPE } from '@helper/types/errors.type';
import { ICurrentAccountEntityFront } from '@helper/types/currentaccount.type';
import { newCurrentAccountSchema } from '../helper/schemaValidator';
...
export class CurrentAccountRouter {
  ...
  private calculateCurrentAccountHandler: RequestHandler = async (req: Request, res: Response) => {
    ...
    try {
      const currentaccount = await this.controller.create(newCurrentAccount);
  ...
  private updateCurrentAccountHandler: RequestHandler = async (req: Request, res: Response) => {
    const { user } = req;
    const { id: currentaccount_id } = req.params;
    const { currentaccount_number } = req.query;
    if (!user?.user) {
      const response: APIResponse<null> = {
        error: {
          error: ERROR_TYPE.BAD_REQUEST,
    ...
    try {
      const currentaccount = await this.controller.get({
        currentaccount_id,
        ...(currentaccount_number && { currentaccount_number: +currentaccount_number }),
      });
  ...
  private getAllCurrentAccountHandler: RequestHandler = async (req: Request, res: Response) => {
    ...
    try {
      const currentaccount = await this.controller.getAll({
