...
export class ActivityDaysRepository {
   ...
   */
  async markDayAsActive(date: string): Promise<void> {
   ...
   */
  async updateActivityCounts(date: string): Promise<void> {
   ...
   */
  async getAllActivityDays(): Promise<IActivityDay[]> {
   ...
   */
  async getActiveDaysOnly(): Promise<IActivityDay[]> {
    const { data, error } = await supabase
      .from('activity_days')
      .select('*')
