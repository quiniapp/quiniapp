...
export class AuthController {
    constructor() {
         ...
         */
        this.loginWithSession = async (props, ipAddress, userAgent) => {
            ...
            if (userData.locked_until && new Date(userData.locked_until) > new Date()) {
                ...
                throw new UnauthorizedError('Cuenta bloqueada por múltiples intentos fallidos. Por favor, contacta al administrador para desbloquear tu cuenta.');
            }
            ...
            if (!isValidPassword) {
                ...
                throw new UnauthorizedError(`Contraseña incorrecta. Te quedan ${remainingAttempts} ${remainingAttempts === 1 ? 'intento' : 'intentos'}.`);
            }
            ...
            const accessToken = signAccessToken(userData.user_id, userData.username, userData.user_type, session.session_id, userData.organization_id);
            const refreshToken = signRefreshToken(userData.user_id, session.session_id, session.refresh_token_version);
         ...
         */
        this.refreshToken = async (refreshToken, ipAddress, userAgent) => {
            ...
            if (!isValidToken) {
                ...
                throw new UnauthorizedError('Actividad sospechosa detectada. Todas las sesiones han sido cerradas.');
            }
            // 5. Get user data
            const userData = await this.repository.getUserById(session.user_id);
            // 6. Generate new tokens (rotate refresh token)
            const newAccessToken = signAccessToken(userData.user_id, userData.username, userData.user_type, session.session_id, session.organization_id);
            const newRefreshToken = signRefreshToken(userData.user_id, session.session_id, session.refresh_token_version + 1);
         ...
         */
        this.logoutSession = async (sessionId, userId, logoutAll = false) => {
            if (logoutAll) {
