...
export class BetRepository {
    async getAllBets({ organization_ids, schedule_id, date, cashier_id, lottery_id, winners, quatern, tern, ticket_number, page = 1, limit = 100, }) {
    ...
    async getAllBetsGrouped({ organization_ids, schedule_id, date, cashier_id, lottery_id, winners, quatern, tern, page = 1, limit = 100, }) {
        ...
        const orgResults = await Promise.all(organization_ids.map(async (orgId) => {
        ...
        for (const orgBets of orgResults) {
            for (const bet of orgBets) {
                ...
                if (mergedMap.has(key)) {
                    ...
                    existing.prize = (existing.prize || 0) + (bet.prize || 0);
                    existing.hits = (existing.hits || 0) + (bet.hits || 0);
                }
                else {
        ...
        let result = Array.from(mergedMap.values()).sort((a, b) => (b.amount || 0) - (a.amount || 0));
        if (quatern && tern) {
            result = result.filter((bet) => bet.bet_type === BET_TYPE.QUATERN || bet.bet_type === BET_TYPE.TERN);
        }
        else if (quatern) {
    ...
    async getTotalAmount({ organization_ids, date, schedule_id, cashier_id, lottery_id, }) {
    ...
    async getTotalPrize({ organization_ids, date, schedule_id, cashier_id, lottery_id, }) {
        // Use direct query with .in() to support multiple org IDs
        const tableName = getTableName(date, 'bets');
        let query = supabase.from(tableName)
            .select('prize.sum()')
    ...
    async getWinnerBets({ organization_id, date, schedule_id, cashier_id, lottery_id, ticket_number, }) {
        // Determine which table to query based on date
        const tableName = getTableName(date, 'bets');
        let query = supabase
            .from(tableName)
    ...
    async getAmountsByTicket({ ticket_number, organization_ids, }) {
        const zeroSums = {
            total_amount: 0,
            total_prize: 0,
            total_count: 0,
