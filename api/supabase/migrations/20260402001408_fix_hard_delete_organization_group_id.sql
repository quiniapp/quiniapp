...
begin
  ...
  -- Quitar el grupo a los usuarios que pertenecían a este grupo
  -- (con el nuevo modelo, organization_id ya no apunta al grupo; group_id sí)
  update public.users
     set group_id = null, edited_at = now()
   where group_id = p_org_id;

  -- Eliminar usuarios cuyo organization_id sea este org (solo aplica a orgs raíz)
