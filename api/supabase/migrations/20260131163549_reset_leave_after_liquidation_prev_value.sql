...
BEGIN
  ...
  --    Mantengo drag "raw" para auditoría, pero calculo rollover (arrastre efectivo)
  previous_state AS (
    SELECT DISTINCT ON (ca.user_id)
      ca.user_id,
      ca.total AS previous_balance,
      ca.drag  AS previous_drag_raw, -- auditoría (el drag real del día anterior)

      -- lo que se arrastra al día siguiente:
      -- si ayer hubo leave y el drag era positivo => rollover = 0
      CASE
        WHEN COALESCE(ca.leave, 0) > 0
         AND COALESCE(ca.drag, 0)  > 0
        THEN 0
        ELSE COALESCE(ca.drag, 0)
  ...
  --    IMPORTANTE: solo manuales. NO usamos previous_balance/previous_drag del día
  existing_day AS (
    SELECT
      ca.user_id,
  ...
  calculated_data AS (
    SELECT
      ...
      -- ✅ previous_balance SIEMPRE desde el estado anterior (no desde la fila del día)
      ...
      -- ✅ drag efectivo (arranque del día) SIEMPRE desde rollover del día anterior
      CASE
  ...
  final_data AS (
    SELECT
      ...
      -- pass total (incluye claims positivos)
      ...
      -- drag calculado del día (acumulado)
      CASE
      ...
      -- leave calculado si corresponde
      CASE
        WHEN cd.fee_plus_pct <= 0 THEN 0
  ...
  resolved AS (
    SELECT
      ...
      -- total final (siempre descuenta leave efectivo)
      (fd.total_pre_leave
       - CASE
           WHEN fd.fee_plus_pct <= 0 THEN 0
           WHEN p_calculate_leave THEN fd.leave_calc
      ...
      -- drag a guardar (auditoría del día, incluye el acumulado real del período)
      CASE
        WHEN fd.fee_plus_pct <= 0 THEN 0
  ...
  upserted_rows AS (
    ...
    ON CONFLICT (user_id, date) DO UPDATE SET
      ...
      -- ✅ previous_* se recalculan (derivados, no manuales)
