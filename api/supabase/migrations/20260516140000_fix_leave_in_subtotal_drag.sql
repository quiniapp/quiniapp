-- Fix: when p_leave_in_subtotal is TRUE, drag must NOT be reduced by leave.
-- Drag = prev_drag + revenue (full revenue, before leave deduction).
-- Only subtotal gets the leave deducted. Total changes because subtotal changes.
    ...
    IF p_calculate_leave THEN
      IF v_drag > 0 THEN
        ...
        IF p_leave_in_subtotal THEN
          -- Leave deducted within subtotal; drag stays as prev_drag + revenue (unchanged)
...
BEGIN
  ...
  resolved AS (
    SELECT
      ...
      -- Drag is always prev_drag + revenue; leave_in_subtotal does not affect drag
      CASE
        WHEN fd.fee_plus_pct <= 0 THEN 0
