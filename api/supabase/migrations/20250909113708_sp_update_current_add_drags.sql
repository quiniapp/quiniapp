...
DECLARE
  ...
  -- flags de p_props
  has_claims        BOOLEAN := (p_props ? 'claims')       AND (p_props->'claims')       IS NOT NULL;
  has_collections   BOOLEAN := (p_props ? 'collections')  AND (p_props->'collections')  IS NOT NULL;
  has_paid          BOOLEAN := (p_props ? 'paid')         AND (p_props->'paid')         IS NOT NULL;
  has_bills         BOOLEAN := (p_props ? 'bills')        AND (p_props->'bills')        IS NOT NULL;

  -- NUEVO: overrides opcionales
  has_prev_drag     BOOLEAN := (p_props ? 'previous_drag') AND (p_props->'previous_drag') IS NOT NULL;
  has_drag          BOOLEAN := (p_props ? 'drag')          AND (p_props->'drag')          IS NOT NULL;
BEGIN
  -- 1) Fila actual (lock)
  SELECT * INTO cur
  ...
  -- NUEVO: override previous_drag si viene desde frontend
  ...
  IF v_fee_plus_pct > 0 THEN
    -- NUEVO: override de drag si viene, si no se calcula
    IF has_drag THEN
    ...
    END IF;
  ELSE
    -- Sin fee_plus no hay drag/leave (respetamos regla original)
    v_prev_drag := 0;
    v_drag      := 0;
    v_leave     := 0;
  END IF;
