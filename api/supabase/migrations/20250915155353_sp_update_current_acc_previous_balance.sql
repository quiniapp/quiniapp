...
DECLARE
  ...
  v_total           NUMERIC := 0;
  v_drag            NUMERIC := 0;
  v_prev_drag       NUMERIC := 0; -- efectivo para hoy
  v_leave           NUMERIC := 0;
  ...
  has_prev_total    BOOLEAN := FALSE;   -- NUEVO: override previous_balance (de ayer)
BEGIN
  -- 1) Fila actual (lock)
  SELECT * INTO cur
  ...
  -- previous_drag EFECTIVO (reset diferido si ayer leave>0 y drag>0)
  v_prev_drag := CASE
  ...
  -- NUEVO: override previous_balance (total del día anterior) si viene desde frontend
  IF has_prev_total THEN
    v_prev_total := (p_props->>'previous_balance')::NUMERIC;
  END IF;

  -- NUEVO: override previous_drag si viene desde frontend (drag efectivo de ayer)
  IF has_prev_drag THEN
    v_prev_drag := (p_props->>'previous_drag')::NUMERIC;
  END IF;
  ...
  -- bills “hoy” viene del día anterior salvo override explícito
  ...
  -- 7) DRAG y LEAVE (condicionado por fee_plus)
  IF v_fee_plus_pct > 0 THEN
    -- override de drag si viene, si no se calcula
    IF has_drag THEN
