...
DECLARE
  ...
  -- manuales efectivos (si llegan en props, usar props; si no, usar lo de la fila actual)
...
BEGIN
  -- 1) Traer y bloquear la fila a actualizar (ES la del día actual para ese usuario)
  SELECT * INTO cur
  ...
  -- 2) Tickets del día (base objetiva)
  SELECT
  ...
  -- 4) Estado anterior (< v_date)
  SELECT
    COALESCE(ca.total, 0),
    COALESCE(ca.drag,  0),
    COALESCE(ca.bills, 0)
  INTO v_prev_total, v_prev_drag, v_prev_bills
  ...
  -- bills: por regla, si viene en props se asigna ese valor; si no, se toma del día anterior
  eff_bills       := COALESCE( (CASE WHEN has_bills       THEN (p_props->>'bills')::NUMERIC       END), v_prev_bills );

  -- 6) Reglas del día (recalcular TODO desde cero)
  claims_pos := GREATEST(eff_claims, 0);
  claims_neg := LEAST(eff_claims, 0);

  v_pass     := v_pass_raw + claims_pos;
  v_comm     := ROUND(v_pass * COALESCE(v_fee_pct, 0), 2);
  v_subtotal := v_pass - v_comm - v_successes + claims_neg;  -- claims<0 resta directo
  v_revenue  := v_subtotal;                                   -- revenue = subtotal

  v_total    := v_prev_total + v_revenue - eff_collections + eff_paid;
  v_drag     := v_prev_drag + v_revenue;

  -- 7) Actualizar la fila y devolver
