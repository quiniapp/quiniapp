...
BEGIN
  ...
  final_data AS (
    SELECT
      ...
      -- previous_drag a guardar hoy (efectivo) o 0 si no hay fee_plus
      CASE
        WHEN cd.fee_plus_pct <= 0 THEN 0
  ...
  -- 6) UPSERT (manuales no se pisan; leave sólo si p_calculate_leave=TRUE)
  upserted_rows AS (
    ...
    ON CONFLICT (user_id, date) DO UPDATE SET
      ...
      -- ✅ liquidación: sólo se sube a TRUE si p_liquidated=TRUE; nunca se baja
      is_liquidated      = CASE
...
-- Defaults/NOT NULL recomendados (si no los tenías ya)
