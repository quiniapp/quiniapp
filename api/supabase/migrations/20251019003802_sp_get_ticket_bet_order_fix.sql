...
begin
  ...
  br as (  -- base rows
    select
      ...
      b.bet_order        as bet_order,              -- ✅ traer bet_order
      s.schedule_id,
  ...
  -- Deduplicación por identidad de bet + schedule + lottery
  br_dedup as (
    select distinct on (
  ...
  -- Por bet => scheduleLottery[]
  bets_grouped as (
    select
      number, amount, place, "with", position,
      min(bet_created_at) as bet_created_at,
      min(bet_order)      as bet_order,            -- ✅ llevar bet_order
      jsonb_agg(
        jsonb_build_object(
  ...
  -- JSON final de bets (ordenado SOLO por bet_order)
  bets_json as (
    select jsonb_agg(
      jsonb_build_object(
        'number',          number,
      ...
      order by bet_order asc                        -- ✅ orden determinista por bet_order
    ) as bets
    from bets_grouped
  )
  select
