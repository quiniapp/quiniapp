...
CREATE OR REPLACE FUNCTION update_current_account_recompute(
  ...
  p_props JSONB,             -- { "claims"?: number, "paid"?: number, "collections"?: number, "bills"?: number }
  p_calculate_leave BOOLEAN  -- si true calcula 'leave' hoy (el reset del drag se aplica al día siguiente)
)
...
DECLARE
  ...
  -- fee usuario
  v_fee_pct         NUMERIC := 0;  -- fee (0..1)
  v_fee_plus_pct    NUMERIC := 0;  -- fee_plus (0..1)
  ...
  v_prev_drag       NUMERIC := 0;  -- previous_drag EFECTIVO para hoy
...
BEGIN
  ...
  -- bills: si viene, usar props; sino, tomar del día anterior
  ...
  IF v_fee_plus_pct > 0 THEN
    -- hay fee_plus → drag se calcula normalmente
    v_drag := v_prev_drag + v_revenue;

    IF p_calculate_leave THEN
      IF v_drag > 0 THEN
        v_leave := ROUND(v_drag * v_fee_plus_pct, 2);
        v_total := v_total - v_leave;  -- descuenta del total
        -- OJO: no se resetea drag hoy; el "reset" se refleja mañana en previous_drag efectivo
      ELSE
        v_leave := 0;
      END IF;
    ELSE
      v_leave := cur.leave; -- conservar si hoy no se recalcula leave
    END IF;

  ELSE
    -- sin fee_plus → drag no se calcula y es 0; leave = 0
    v_prev_drag := 0;
    v_drag      := 0;
    v_leave     := 0;
    -- v_total NO se ajusta por leave
  END IF;
  ...
  SET
    ...
    previous_drag       = v_prev_drag,  -- ya condicionado por fee_plus y reset diferido
    total               = v_total,
    drag                = v_drag,       -- 0 si no hay fee_plus
    leave               = v_leave,      -- 0 si no hay fee_plus
    edited_at           = NOW()
