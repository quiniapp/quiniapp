-- ============================================================================
-- Add DB-side pagination and bet_type filter to grouped bets RPCs.
-- Replaces JS-side pagination (fetch all rows, slice in memory).
--
-- New parameters:
--   p_bet_types  bet_type_enum[]  — filter to specific types (NULL = all)
--   p_limit      INT              — page size (default 100)
--   p_offset     INT              — row offset (default 0)
--
-- New count RPCs (same filters, returns total matching groups):
--   get_grouped_bets_count
--   get_grouped_bets_count_archive
-- ============================================================================

DROP FUNCTION IF EXISTS public.get_grouped_bets_for_parse(DATE, UUID, UUID, UUID, BOOLEAN, UUID, UUID[], NUMERIC);
DROP FUNCTION IF EXISTS public.get_grouped_bets_for_parse_archive(DATE, UUID, UUID, UUID, BOOLEAN, UUID, UUID[], NUMERIC);

-- ============================================================================
-- 1. get_grouped_bets_for_parse  (main table, paginated)
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.get_grouped_bets_for_parse(
  ...
  p_bet_types       bet_type_enum[]  DEFAULT NULL,
...
AS $$
  SELECT
    ...
    (SELECT to_jsonb(l2) FROM public.lotteries l2
      WHERE l2.lottery_id = b.lottery_id
        AND (p_organization_id IS NULL OR l2.organization_id = p_organization_id)) AS lotteries,
    (SELECT to_jsonb(s2) FROM public.schedules s2
      WHERE s2.schedule_id = b.schedule_id
        AND (p_organization_id IS NULL OR s2.organization_id = p_organization_id)) AS schedules
  FROM public.bets b
  WHERE b.date = p_date
    ...
    AND (p_bet_types IS NULL OR b.bet_type = ANY(p_bet_types))
    AND (
...
-- ============================================================================
-- 2. get_grouped_bets_count  (main table, count only — for pagination header)
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_grouped_bets_count(
  p_date            DATE,
  p_schedule_id     UUID             DEFAULT NULL,
  p_cashier_id      UUID             DEFAULT NULL,
  p_lottery_id      UUID             DEFAULT NULL,
...
-- ============================================================================
-- 3. get_grouped_bets_for_parse_archive  (archive table, paginated)
-- ============================================================================
...
-- ============================================================================
-- 4. get_grouped_bets_count_archive
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_grouped_bets_count_archive(
  p_date            DATE,
  p_schedule_id     UUID             DEFAULT NULL,
  p_cashier_id      UUID             DEFAULT NULL,
  p_lottery_id      UUID             DEFAULT NULL,
