...
BEGIN
  ...
  calculated_data AS (
    SELECT
      ...
      -- FIX: Check if PREVIOUS day was liquidated, not if CURRENT day is liquidating
     CASE
  WHEN COALESCE(u.fee_plus, 0) <= 0 THEN 0
  WHEN COALESCE(ps.previous_is_liquidated, FALSE) = TRUE
   AND COALESCE(ps.previous_leave, 0) > 0
