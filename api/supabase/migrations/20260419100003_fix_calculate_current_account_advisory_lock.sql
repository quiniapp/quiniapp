...
-- Date: 2026-04-19
-- Two admins sharing an account clicking "liquidar" simultaneously for the same org+date
-- could see each other's partially-committed writes. The advisory lock serializes the
-- calculate step per (org, date) so the second caller waits until the first commits.
...
DECLARE
  ...
  result_array JSONB[];
BEGIN
  -- Serialize concurrent calculate calls for the same org+date.
  -- Prevents two admins from running this simultaneously and reading each other's
  -- partially-committed update_current_account_recompute writes.
