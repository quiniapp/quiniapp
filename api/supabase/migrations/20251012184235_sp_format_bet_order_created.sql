...
begin
  ...
  br as (
    select
      ...
      b."with",
      b.position,
      b.created_at                                   as bet_created_at,  -- 👈 para ordenar bets
  ...
  bet_sched as (
    select
      ...
      min(bet_created_at) as bet_created_at,  -- 👈 una sola marca de tiempo por bet-identity
  ...
  bets_grouped as (
    select
      number, amount, place, "with", position,
      min(bet_created_at) as bet_created_at,  -- 👈 conservar para el orden final
      jsonb_agg(
        jsonb_build_object(
  ...
  -- Orden final: por fecha de creación de la bet (ASC) y como tie-breaker por número
  bets_json as (
    select jsonb_agg(
      jsonb_build_object(
        'number',          number,
      ...
      order by bet_created_at asc, number
    ) as bets
    from bets_grouped
  )
  select
