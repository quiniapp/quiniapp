...
BEGIN
  ...
  LOOP
    ...
    ELSIF bet.bet_type = 'QUATERN' THEN
      RAISE LOG ' Entrando a bet_type = QUATERN';
      IF bet.place = 'HEAD' THEN
        RAISE LOG ' Entrando a place = HEAD';
        IF calc_results[1] = bet.number THEN
          payout := 3500 * bet.amount;
        END IF;
      ELSIF bet.place = 'FIVE' THEN
        RAISE LOG ' Entrando a place = FIVE';
        match_count := 0;
        FOR i IN 1..5 LOOP
      ...
      ELSIF bet.place = 'TEN' THEN
        RAISE LOG ' Entrando a place = TEN';
        match_count := 0;
        FOR i IN 1..10 LOOP
          IF calc_results[i] = bet.number THEN
            RAISE LOG '    Coincidencia en TEN, posición % para QUATERN. Numero: %, Resultado: %', i, bet.number, calc_results[i];
            match_count := match_count + 1;
          END IF;
        END LOOP;
        payout := 350 * bet.amount * match_count;
      ELSIF bet.place = 'TWENTY' THEN
        RAISE LOG ' Entrando a place = TWENTY';
        match_count := 0;
        FOR i IN 1..10 LOOP
          IF calc_results[i] = bet.number THEN
            match_count := match_count + 1;
