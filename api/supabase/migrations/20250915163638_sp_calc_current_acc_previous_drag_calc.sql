...
BEGIN
  ...
  -- 3) Día existente (si ya hay fila: traigo manuales y también previous_* del mismo día)
  existing_day AS (
    SELECT
      ca.user_id,
  ...
  -- 4) Cálculo según reglas (con prioridad para previous_* del día si existe)
  calculated_data AS (
    SELECT
      u.user_id,
      u.name   AS user_name,
      ...
      -- previous_* elegidos (prioridad: existentes hoy -> histórico -> 0)
      ...
      -- previous_drag efectivo elegido (si hay fee_plus: prioridad existente hoy -> hist. efectivo)
      CASE
        WHEN COALESCE(u.fee_plus, 0) <= 0 THEN 0
        ELSE COALESCE(ed.previous_drag_today,
                      CASE
                        WHEN COALESCE(ps.previous_leave, 0) > 0 AND COALESCE(ps.previous_drag_raw, 0) > 0 THEN 0
                        ELSE COALESCE(ps.previous_drag_raw, 0)
                      END,
  ...
  final_data AS (
    SELECT
      ...
      -- por prolijidad, también exponemos cashier_commission/pass calculados
      ...
      -- bills: siempre del día anterior (se mantiene la regla original)
      ...
      -- drag calculado con el previous_drag EFECTIVO elegido
      CASE
  ...
  upserted_rows AS (
    ...
    SELECT
      ...
      CASE WHEN p_calculate_leave THEN r.leave_calc ELSE r.leave_manual END,
      v_date,
      NOW(), NOW(),
