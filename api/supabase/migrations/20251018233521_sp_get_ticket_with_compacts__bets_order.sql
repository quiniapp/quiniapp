...
-- br: agregar bet_order
br as (
  select
    b.ticket_id,
    b.number, b.amount, b.place, b."with", b.position,
    b.created_at                     as bet_created_at,
    b.bet_order                      as bet_order,        -- ✅
    s.schedule_id, s."time"          as schedule_time,
    jsonb_build_object(
  ...
  -- Deduplico por (bet-identity + schedule + lottery), quedándome con la lottery más nueva
  br_dedup as (
    select distinct on (
...
-- json final: ordenar solo por bet_order
bets_json as (
  select jsonb_agg(
    jsonb_build_object(
      'number',          number,
    ...
    order by bet_order asc, number             -- ✅ clave del asunto
  ) as bets
  from bets_grouped
)
  select
