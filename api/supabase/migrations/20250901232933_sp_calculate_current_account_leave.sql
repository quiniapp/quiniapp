...
BEGIN
  ...
  calculated_data AS (
    SELECT
      ...
      COALESCE(ed.leave, 0)       AS leave_manual,   -- lo manual del día
      ...
      -- previous_drag EFECTIVO: reset diferido si ayer hubo leave>0 y drag>0; si no hay fee_plus => 0
      CASE
  ...
  final_data AS (
    SELECT
      ...
      -- drag: si no hay fee_plus => 0; si hay => previous_drag_eff + revenue
      CASE
        WHEN cd.fee_plus_pct <= 0 THEN 0
        ELSE cd.previous_drag_eff + cd.revenue
      END AS drag_calc,

      -- leave calculado (solo si se pide y hay fee_plus; si no, conservar manual o 0)
      CASE
        WHEN cd.fee_plus_pct <= 0 THEN 0
      ...
      -- total final (si calculamos leave hoy, lo descontamos; si no, queda igual)
      ...
      -- previous_drag que guardamos hoy: el EFECTIVO (con reset diferido ya aplicado),
  ...
  upserted_rows AS (
    ...
    SELECT
      ...
      -- leave: si calculamos hoy, guardar el calculado; si no, conservar manual
      CASE
