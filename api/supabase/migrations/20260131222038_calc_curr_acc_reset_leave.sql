...
BEGIN
  ...
  --    Resetea drag si el día anterior tuvo liquidación con leave > 0
  previous_state AS (
    SELECT DISTINCT ON (ca.user_id)
      ...
      -- ✅ rollover: si el día anterior está LIQUIDADO y se calculó leave, arranca en 0
      CASE
        WHEN COALESCE(ca.is_liquidated, FALSE) = TRUE
         AND COALESCE(ca.leave, 0) > 0
        THEN 0
