...
BEGIN
  ...
  --    Mantengo drag real para auditoría, pero calculo rollover para el día siguiente
  previous_state AS (
    SELECT DISTINCT ON (ca.user_id)
      ca.user_id,
      ca.total AS previous_balance,
      ca.drag  AS previous_drag_raw, -- auditoría

      -- ✅ rollover: si el día anterior está LIQUIDADO y el drag era positivo, arranca en 0
      CASE
        WHEN COALESCE(ca.is_liquidated, FALSE) = TRUE
  ...
  -- 3) Día existente (si ya hay fila) -> SOLO manuales
  existing_day AS (
    SELECT
      ca.user_id,
  ...
  calculated_data AS (
    SELECT
      ...
      -- ✅ previous_balance SIEMPRE desde el estado anterior
      ...
      -- ✅ previous_drag efectivo SIEMPRE desde rollover
      CASE
  ...
  resolved AS (
    SELECT
      ...
      -- drag guardado (auditoría del día)
      CASE
        WHEN fd.fee_plus_pct <= 0 THEN 0
