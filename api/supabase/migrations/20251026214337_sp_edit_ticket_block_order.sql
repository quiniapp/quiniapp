...
begin
  ...
  numbered as (
    -- bet_group_order: orden por BLOQUE (índice del bet original)
    -- combo_rn: orden interno dentro del bloque (opcional)
    select
      c.*,
  ...
  select
    ...
    bet_order, created_at
  from prepared
  order by bet_order, combo_rn;  -- escritura estable dentro de cada bloque
