...
export const BetTableSchema = z
    ...
    if (len === 1) {
        if (data.with)
            addIssue('with', 'No debe estar definido para número de 1 dígito');
        if (data.position)
            addIssue('position', 'No debe estar definido para número de 1 dígito');
    }
    ...
    if (len === 2) {
        if (withLen === 0) {
            ...
            if (!data.position)
                addIssue('position', 'Para re-doble, "position" debe estar definida');
            const validPositionsByPlace = {
            ...
            if (data.position &&
                ...
                addIssue('position', `Para place ${data.place}, position debe ser una de: ${validPositionsByPlace[data.place].join(', ')}`);
            }
        }
    }
    // TERN (3 dígitos)
    if (len === 3) {
        if (data.with)
            addIssue('with', 'No debe estar definido para número de 3 dígitos');
        if (data.position)
            addIssue('position', 'No debe estar definido para número de 3 dígitos');
    }
    // QUATERN (4 dígitos)
    if (len === 4) {
        if (data.with)
            addIssue('with', 'No debe estar definido para número de 4 dígitos');
        if (data.position)
            addIssue('position', 'No debe estar definido para número de 4 dígitos');
    }
